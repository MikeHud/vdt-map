...
- `style.css` — styles the Leaflet popup content and the two custom marker types (`.stopover-dot`, `.current-location-marker` with CSS `pulse` keyframe animation), plus the side panel (`#panel`, `#panel-toggle`, list items, selection/highlight states).
